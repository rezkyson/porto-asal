...
<template>
  ...
  >
    ...
    <div 
      class="relative flex items-center gap-2 px-2 py-2 rounded-full border transition-all duration-500"
      :class="[
        isScrolled 
    ...
    >
      ...
      <div v-if="!isScrolled" class="absolute inset-0 bg-white/5 backdrop-blur-md rounded-full border border-white/5 -z-10"></div>

      <!-- Logo Area -->
      <a href="#" class="pl-4 pr-6 flex items-center gap-2 group">
        <div class="relative w-8 h-8 flex items-center justify-center bg-gradient-to-tr from-blue-500 to-purple-600 rounded-lg overflow-hidden transition-transform duration-300 group-hover:rotate-12">
            <span class="text-white font-bold font-display text-sm">R</span>
        </div>
        <span class="font-display font-bold text-white tracking-widest text-sm group-hover:text-blue-300 transition-colors">
      ...
      <!-- Call to Action (Mini) -->
      <a href="mailto:contact@rezky.com" class="hidden md:flex items-center justify-center w-10 h-10 rounded-full bg-white text-black hover:scale-110 transition-transform ml-2">
      ...
      <!-- Mobile Menu Button -->
      <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="md:hidden p-3 text-white rounded-full hover:bg-white/10 transition-colors">
        <div class="w-5 h-4 flex flex-col justify-between">
           <span class="w-full h-0.5 bg-current transform transition-all" :class="isMobileMenuOpen ? 'rotate-45 translate-y-1.5' : ''"></span>
    ...
    >
      <div v-if="isMobileMenuOpen" class="absolute top-24 left-4 right-4 bg-[#0B1120]/90 backdrop-blur-2xl border border-white/10 rounded-3xl p-4 flex flex-col gap-2 shadow-2xl md:hidden">
         <a v-for="link in links" :key="link.name" :href="link.href" 
             ...
             <span class="w-2 h-2 rounded-full bg-blue-500 opacity-0 group-hover:opacity-100 transition-opacity shadow-[0_0_10px_rgba(59,130,246,0.8)]"></span>
          </a>
      </div>
    </Transition>
  </nav>
